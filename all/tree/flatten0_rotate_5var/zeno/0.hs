...
data Tree = TNil | Node Tree Nat Tree
instance Eq Tree where
  eq (TNil) (TNil) = True
  eq (Node a0 x a1) (Node b0 y b1) = (and (and (and True (eq a0 b0)) (eq x y)) (eq a1 b1))
  eq _ _ = False
...
flatten0 TNil = Nil
flatten0 (Node p x q) = ((++) (flatten0 p) (Cons x (flatten0 q)))

prop p q r x y = proveBool $  (eq (flatten0 (Node (Node p x q) y r)) (flatten0 (Node p x (Node q y r))))
